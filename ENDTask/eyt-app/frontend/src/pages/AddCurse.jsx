...
export function AddCurse() {
  ...
  const agregarCurso = (e) => {
    ...
    localStorage.setItem("eyt_courses", JSON.stringify(cursosActualizados));
  ...
  const eliminarCurso = (id) => {
    const cursosActualizados = cursos.filter((c) => c.id !== id);
    setCursos(cursosActualizados);
    localStorage.setItem("eyt_courses", JSON.stringify(cursosActualizados));
  };
                ...
                {COLORES_CURSOS.map((color, index) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full transition-all ${
                      colorIndex === index ? "ring-2 ring-offset-2" : "hover:scale-110"
                    }`}
                    style={{
                      background: color,
                      ringColor: color,
