...
const COLORES     = { "Crítica":"#e05252","Alta":"#e09052","Media":"#c8a84b","Baja":"#52a8e0" };
...
export function TaskForm({ onTaskCreated }) {
  ...
  // ✅ Cargar tareas desde MongoDB al abrir la pantalla
  useEffect(() => {
    tasksAPI.getAll()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ Guardar tarea en MongoDB via backend
                ...
                {[["Inicio","inicioDia","inicioMes"],["Fin","finDia","finMes"]].map(([lbl,dF,mF]) => (
                  <div key={lbl} className="flex flex-col gap-1">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[[dF, DIAS, "Día"],[mF, MESES, "Mes"]].map(([field, opts, ph]) => (
                ...
                <div className="flex items-center gap-3 mt-2 flex-wrap">
                  {t.fechaFin && (
                    <span className="text-xs text-gray-400">
                      📅 {new Date(t.fechaFin).toLocaleDateString("es-CO")}
                    </span>
                  )}
                  {t.etiquetas?.map((tag) => (
