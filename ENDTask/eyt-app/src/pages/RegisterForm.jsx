...
export function RegisterForm({ onSuccess, showToast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.password !== form.confirm) errs.confirm  = "Las contrasenas no coinciden";
    if (!accepted)                      errs.terms    = "Debes aceptar los terminos";
    ...
    try {
      const data = await authAPI.register({
        ...
        anio: Number(form.anio), password: form.password,
      });
      showToast("Registro exitoso! Bienvenido/a, " + data.user.nombre, "success");
    ...
    <form onSubmit={handleSubmit} className="px-10 pb-10 flex flex-col gap-3">
      <div className="grid grid-cols-2 gap-3">
        <Field label="Nombre" icon={<IconUser />} error={errors.nombre}>
          <input type="text" required value={form.nombre} onChange={ch("nombre")} placeholder="Juan"
        ...
        <Field label="Apellido" icon={<IconUser />} error={errors.apellido}>
          <input type="text" required value={form.apellido} onChange={ch("apellido")} placeholder="Garcia"
            className="bg-transparent w-full text-sm text-gray-800 outline-none placeholder-gray-400" />
        </Field>
      </div>
      <Field label="Telefono" icon={<IconPhone />} error={errors.telefono}>
        <input type="tel" required value={form.telefono} onChange={ch("telefono")} placeholder="+57 300 000 0000"
          className="bg-transparent flex-1 text-sm text-gray-800 outline-none placeholder-gray-400" />
      </Field>
      <Field label="Correo electronico" icon={<IconMail />} error={errors.email}>
        <input type="email" required value={form.email} onChange={ch("email")} placeholder="correo@ejemplo.com"
          className="bg-transparent flex-1 text-sm text-gray-800 outline-none placeholder-gray-400" />
      </Field>
      <Field label="Ano de nacimiento" icon={<IconCalendar />} error={errors.anio}>
        <select required value={form.anio} onChange={ch("anio")}
          ...
          <option value="" disabled>Selecciona un ano</option>
          {years.map((y) => <option key={y} value={y} style={{ color: "#1f2937" }}>{y}</option>)}
        </select>
      ...
      <PasswordField label="Contrasena" value={form.password} onChange={ch("password")}
        placeholder="Min. 8 caracteres" show={showPw} onToggle={() => setShowPw(!showPw)} error={errors.password} />
      <PasswordField label="Confirmar contrasena" value={form.confirm} onChange={ch("confirm")}
        placeholder="Repite tu contrasena" show={showCf} onToggle={() => setShowCf(!showCf)} error={errors.confirm} />
      <label className="flex items-start gap-2 cursor-pointer select-none">
        ...
        <span className="text-xs text-gray-500">Acepto los <span className="underline text-yellow-600 cursor-pointer">Terminos y Condiciones</span></span>
      </label>
      {errors.terms && <span className="text-xs text-red-500 -mt-1">{errors.terms}</span>}
      <button type="submit" disabled={loading}
        className="mt-1 w-full py-3.5 rounded-xl text-sm font-bold text-white tracking-widest shadow-md active:scale-95 transition-transform disabled:opacity-60"
